...
// runService 启动完整的服务（DB → 缓存 → 用户 → HTTP Server → 后台任务），
// 并在 shutdown.Listen 阻塞直到退出。onReady 在 HTTP Server 启动成功后调用，
// 供桌面模式等场景挂接「打开浏览器 / 系统托盘」等动作。
func runService(cmd *cobra.Command, onReady func(addr string)) {
