...
// 且库 > 64MB」判定是否值得回收；VACUUM 需要独占写锁，调用方（每日任务）
// 已与其它周期写者错相。MySQL/Postgres 直接跳过。
func DBVacuumIfNeeded(ctx context.Context) (bool, error) {
	...
	if err := dbConn.Raw("PRAGMA page_count").Scan(&pageCount).Error; err != nil {
		return false, err
	}
	if err := dbConn.Raw("PRAGMA freelist_count").Scan(&freeListCount).Error; err != nil {
		return false, err
	}
	if err := dbConn.Raw("PRAGMA page_size").Scan(&pageSize).Error; err != nil {
		return false, err
	}
	...
	if err := dbConn.Exec("VACUUM").Error; err != nil {
		return false, err
	}
