...
// TransformStream 旧块式接口的委托实现：唯一编码状态机在 TransformStreamEvents。
// 此入口仅服务 volcengine（未实现事件接口）× anthropic 入站组合的 fallback 路径，
// 块到事件的转换由 StreamEventsFromInternalResponse 承担（error/[DONE]/迟到 usage
// 的终结语义在事件编码器中均已覆盖）。
